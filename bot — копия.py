...
def get_main_keyboard():
    ...
    active_categories = list(PRICES.keys()) + SPECIAL_CATEGORIES
    
    # Формируем кнопки в два столбца
    for i in range(0, len(active_categories), 2):
        row = []
        for cat in active_categories[i:i+2]:
            emoji = CATEGORY_EMOJIS.get(cat, "")
            # Создаем безопасный callback_data (только латиница, цифры, подчеркивание)
            safe_cat = re.sub(r'[^a-zA-Z0-9_]', '_', cat)
            row.append(InlineKeyboardButton(f"{emoji} {cat}".strip(), callback_data=f"cat_{safe_cat}"))
        if row:
            keyboard.append(row)
...
Ваш ТехноДруг всегда на связи! 🤖"""
    ...
    if data.startswith("cat_"):
        ...
        if category == "FAQ / Гарантия":
            ...
            await query.edit_message_text(
                f"📋 *FAQ / Гарантия*\n\n{faq_text}\n\n"
                f"Ваш ТехноДруг всегда на связи! 🤖",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
        ...
        if category == "Предзаказ":
            ...
            # Используем parse_mode=None, чтобы избежать ошибок парсинга
            await query.edit_message_text(
                f"📦 *Предзаказ*\n\n{preorder_text}",
                parse_mode=None,
                reply_markup=InlineKeyboardMarkup([
